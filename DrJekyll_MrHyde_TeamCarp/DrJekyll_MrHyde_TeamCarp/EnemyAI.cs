// Created this class | Julius 18-11-21
using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Graphics;
using Microsoft.Xna.Framework.Content;
using Microsoft.Xna.Framework.Input;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using DrJekyll_MrHyde_TeamCarp;

namespace DrJekyll_MrHyde_TeamCarp
{
    class EnemyAI
    {
        // Texture & Rectangle | Julius 18-12-03
        private Texture2D enemySpriteSheet;
        private Texture2D enemySpriteSheetNM;
        private int spriteWidth = 39;
        private int spriteHeight = 69;
        private Rectangle enemyRectangle;
        private Rectangle spriteRectangle;
        private Rectangle collisionRectangle;

        // Position & Speed | Julius 18-12-03
        public Vector2 enemyPosition = new Vector2(200, 200);
        private Vector2 velocity;
        private int speed = 100;
        // Ai variables
        private bool playerOnRight, playerOnLeft, playerOver, playerUnder;
        Player player;
        Vector2 centerPoint;

        // Animation & Keyboard | Julius 18-12-03
        private int currentFrame = 0;
        private float animationSpeed = 200f;
        private float timer = 0f;
        KeyboardState currentKBState;
        KeyboardState previousKBState;


        public Rectangle CollisionRectangle
        {
            get { return collisionRectangle; }
            set { collisionRectangle = value; }
        }


        // A get/set to share players Position with other clases | Julius 18-12-03
        public Vector2 Position
        {
            get { return enemyPosition; }
            set { enemyPosition = value; }
        }
        // A function that creates a new player | Julius 18-12-03
        public EnemyAI(Texture2D texture, Texture2D textureNM, int currentFrame, int spriteWidth, int spriteHeight, Player player)
        {
            this.enemySpriteSheet = texture;
            this.enemySpriteSheetNM = textureNM;
            this.currentFrame = currentFrame;
            this.spriteWidth = spriteWidth;
            this.spriteHeight = spriteHeight;
            this.player = player;
        }






        

        public void Update(GameTime gameTime, Player player)
        {
            this.player = player;
            // Update Position by adding velocity every game tic | Julius 18-12-03
            enemyPosition += velocity;
            // A rectangle for the sprite of the player | Julius 18-12-03
            enemyRectangle = new Rectangle((int)enemyPosition.X, (int)enemyPosition.Y, spriteWidth, spriteHeight);
            // A rectangle for the collision of the player, which is a little lower down and is a square | Julius 18-12-03
            collisionRectangle = new Rectangle((int)enemyPosition.X, (int)enemyPosition.Y + spriteHeight - spriteWidth, spriteWidth, spriteWidth);
            // Update enemyCenter to its centerpoint of hitbox | Julius 18-12-04
            centerPoint = new Vector2(collisionRectangle.X / 2, collisionRectangle.Y / 2);

            ArtificialIntelligence(gameTime);
            Input(gameTime);
        }



        private void ArtificialIntelligence(GameTime gameTime)
        {
            if (player.centerPoint.X >= centerPoint.X)
            {
                playerOnRight = true;
                playerOnLeft = false;
            }

            if (player.centerPoint.X <= centerPoint.X)
            {
                playerOnLeft = true;
                playerOnRight = false;
            }

            if (player.centerPoint.Y >= centerPoint.Y)
            {
                playerUnder = true;
                playerOver = false;
            }

            if (player.centerPoint.Y <= centerPoint.Y)
            {
                playerOver = true;
                playerUnder = false;
            }


        }


        // Movement | Julius 18-12-03
        private void Input(GameTime gameTime)
        {

            previousKBState = currentKBState;
            currentKBState = Keyboard.GetState();

            spriteRectangle = new Rectangle(currentFrame * spriteWidth, 0, spriteWidth, spriteHeight);
            //Which spriteframes that are conected to each other //Carl 15/11/18
            if (currentKBState.GetPressedKeys().Length == 0)
            {
                if (currentFrame > 0 && currentFrame < 4)
                    currentFrame = 0;

                if (currentFrame > 4 && currentFrame < 8)
                    currentFrame = 4;

                if (currentFrame > 8 && currentFrame < 12)
                    currentFrame = 8;

                if (currentFrame > 12 && currentFrame < 16)
                    currentFrame = 12;
            }


            // Animate walk and change Position when going Left or Right | Julius 18-12-03
            if (playerOnLeft == true)
            {
                // "speed" representates how many pixels per second the player is moving | Julius 18-12-03
                velocity.X = -(float)gameTime.ElapsedGameTime.TotalMilliseconds * speed / 1000;
                // Animate left | Julius 18-12-03
                if (currentKBState.IsKeyDown(Keys.A) == true || currentKBState.IsKeyDown(Keys.Left) == true)
                    AnimateLeft(gameTime);
            }
            else if (playerOnRight == true)
            {
                // "speed" representates how many pixels per second the player is moving | Julius 18-12-03
                velocity.X = (float)gameTime.ElapsedGameTime.TotalMilliseconds * speed / 1000;
                // Animate right | Julius 18-12-03
                if (currentKBState.IsKeyDown(Keys.D) == true || currentKBState.IsKeyDown(Keys.Right) == true)
                    AnimateRight(gameTime);
            }
            // If no button is pressed than the player shall stop moving on the X-axis | Julius 18-12-03
            else velocity.X = 0f;

            // Animate walk and change Position when going Up or Down | Julius 18-12-03
            if (playerOver == true)
            {
                // "speed" representates how many pixels per second the player is moving | Julius 18-12-03
                velocity.Y = -(float)gameTime.ElapsedGameTime.TotalMilliseconds * speed / 1000;
                // Animate up | Julius 18-12-03
                if (currentKBState.IsKeyDown(Keys.W) == true || currentKBState.IsKeyDown(Keys.Up) == true)
                    AnimateUp(gameTime);
            }
            else if (playerUnder == true)
            {
                // "speed" representates how many pixels per second the player is moving | Julius 18-12-03
                velocity.Y = (float)gameTime.ElapsedGameTime.TotalMilliseconds * speed / 1000;
                // Animate down | Julius 18-12-03
                if (currentKBState.IsKeyDown(Keys.S) == true || currentKBState.IsKeyDown(Keys.Down) == true)
                    AnimateDown(gameTime);
            }
            // If no button is pressed than the player shall stop moving on the Y-axis | Julius 18-12-03
            else velocity.Y = 0f;

        }

        // Animation code by Carl | Julius 18-11-28
        #region Animation
        //Kod för att frame 1 till 4 används när spelaren går neråt //Carl 07/11/18
        public void AnimateUp(GameTime gameTime)
        {
            if (currentKBState != previousKBState)
            {
                currentFrame = 9;
            }

            timer += (float)gameTime.ElapsedGameTime.TotalMilliseconds;

            if (timer > animationSpeed)
            {
                currentFrame++;

                if (currentFrame > 11)
                {
                    currentFrame = 8;
                }
                timer = 0f;
            }
        }
        //Kod för att frame 5 till 8 används när spelaren går åt höger //Carl 07/11/18
        public void AnimateRight(GameTime gameTime)
        {
            if (currentKBState != previousKBState)
            {
                currentFrame = 5;
            }

            timer += (float)gameTime.ElapsedGameTime.TotalMilliseconds;

            if (timer > animationSpeed)
            {
                currentFrame++;

                if (currentFrame > 7)
                {
                    currentFrame = 4;
                }
                timer = 0f;
            }
        }
        //Kod för att frame 9 till 12 används när spelaren går uppåt //Carl 07/11/18
        public void AnimateDown(GameTime gameTime)
        {
            if (currentKBState != previousKBState)
            {
                currentFrame = 1;
            }

            timer += (float)gameTime.ElapsedGameTime.TotalMilliseconds;

            if (timer > animationSpeed)
            {
                currentFrame++;

                if (currentFrame > 3)
                {
                    currentFrame = 0;
                }
                timer = 0f;
            }
        }
        //Kod för att frame 13 till 16 används när spelaren går åt vänster //Carl 07/11/18
        public void AnimateLeft(GameTime gameTime)
        {
            if (currentKBState != previousKBState)
            {
                currentFrame = 13;
            }

            timer += (float)gameTime.ElapsedGameTime.TotalMilliseconds;

            if (timer > animationSpeed)
            {
                currentFrame++;

                if (currentFrame > 15)
                {
                    currentFrame = 12;
                }
                timer = 0f;
            }
        }

        #endregion



        // Kollision för spelaren med tiles och gameborders, newRectangle är vad som ska kollidera med tiles och border, border skrivs i värderna x, x, y, y | Julius 18-11-21
        // Fixade kollisionen som såg ful ut | Julius 18-12-03
        public void Collision(Rectangle newRectangle, int leftBorder, int rightBorder, int topBorder, int bottomBorder)
        {
            // If playerCollisionRectangle touch on the top of TileRectangle | Julius 18-12-03
            if (collisionRectangle.TouchTopOf(newRectangle))
                // Changes Position which makes the player stop from walking through the tile | Julius 18-12-03
                enemyPosition.Y = newRectangle.Y - spriteRectangle.Height -1;

            // If playerCollisionRectangle touch on the left of TileRectangle | Julius 18-12-03
            if (collisionRectangle.TouchLeftOf(newRectangle))
                // Changes Position which makes the player stop from walking through the tile | Julius 18-12-03
                enemyPosition.X = newRectangle.X - spriteRectangle.Width -1;

            // If playerCollisionRectangle touch on the right of TileRectangle | Julius 18-12-03
            if (collisionRectangle.TouchRightOf(newRectangle))
                // Changes Position which makes the player stop from walking through the tile | Julius 18-12-03
                enemyPosition.X = newRectangle.X + newRectangle.Width +1;

            // If playerCollisionRectangle touch on the bottom of TileRectangle | Julius 18-12-03
            if (collisionRectangle.TouchBottomOf(newRectangle))
                // Changes Position which makes the player stop from walking through the tile | Julius 18-12-03
                enemyPosition.Y = newRectangle.Y + newRectangle.Height - spriteRectangle.Height + collisionRectangle.Height;

            // Four if-states which declares that the playerCollisionRectangle shall stay within these assigned borders | Julius 18-12-03
            if (enemyPosition.X < leftBorder) enemyPosition.X = leftBorder;
            if (enemyPosition.X > rightBorder - collisionRectangle.Width) enemyPosition.X = rightBorder - collisionRectangle.Width;
            if (enemyPosition.Y + spriteHeight - spriteWidth < topBorder) enemyPosition.Y = topBorder - spriteHeight + spriteWidth;
            if (enemyPosition.Y > bottomBorder - collisionRectangle.Height) enemyPosition.Y = bottomBorder - collisionRectangle.Width;

        }


        public void Draw(SpriteBatch spriteBatch)
        {
            // Draw the player | Julius 18-12-03
            spriteBatch.Draw(enemySpriteSheet, enemyRectangle, spriteRectangle, Color.White);

        }
        public void DrawNM(SpriteBatch spriteBatch)
        {
            spriteBatch.Draw(enemySpriteSheetNM, enemyRectangle, spriteRectangle, Color.White);
        }

    }
}
